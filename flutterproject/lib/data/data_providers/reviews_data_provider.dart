...
class ReviewsDataProvider {
  ...
  Future getReviewsByCarId(String carId) async {
    try {
      final response =
          await http.get(Uri.parse("$_baseUrl/reviews?CarId=$carId"));
      if (response.statusCode == 200) {
        return response.body;
      }
      throw Exception('Failed to load reviews under CarId: $carId');
    } catch (e) {
      throw e.toString();
    }
  }
